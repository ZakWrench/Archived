...
set.seed(123)
index <- sample(2, nrow(lesdonnees), prob = c(0.3, 0.7), replace = TRUE) #2, : generating samples of ones and twos. nrow(x) means supplying the number of rows in x to generate. prob = c(0.3,0.7) splitting the dataset. Replace = TRUE means that the index will stay the same when used for later.
