Diabetes <- read.csv("C:\\Users\\sofia noussi\\Desktop\\csv\\Final\\Final - relabeling - Copy - Copy.csv", header = T)
#loading libraries#
...
levels(Diabetes$Etat) <- 0:1
head(Diabetes)

#Dividing randomly data samples into train and test dataset#

set.seed(123) #setting random seed
index <- sample(2, nrow(Diabetes), replace = TRUE, prob = c(0.8,0.2)) #obtaining the index 
...
model_logi <-glm(Etat~Filliere+Niveau+Age+Gender+Bourse+Mention+VillePays, data = train, family = "binomial") #predicting binomial outcome
model_logi <-glm(Etat~Filliere+Niveau+Age+Gender+Bourse+Mention+VillePays+ObtentionBac, data = train, family = "binomial") #predicting binomial outcome
...
visreg(model_logi, "Niveau", xlab = "Niveau level", ylab ="Log odds (Etat)") #how to logodds of diabetes changes in respect to glucose level

#Logodds of diabetes wrt to pedigree level
visreg(model_logi, "Filliere", xlab="Filliere level", ylab = "Log odds (Etat)")

#Probabilities of diabetes wrt glucose
visreg(model_logi, "Niveau", scale = "response", rug= 2, xlab = "Glucose level", ylab= "P(Etat)") # the line above and below represent the rug line

#Probabilities of diabetes wrt pedigree
visreg(model_logi, "Filliere", scale = "response", rug = 2, xlab = "pedigree level", ylab = "P(Etat)")
...
head(data.frame(observed = test$Etat, predicted = predicted))

#Let's create a contigency table
tab <- table(predicted, test$Etat)
tab
...
pred.rocr <- prediction(pred, test$Etat)
