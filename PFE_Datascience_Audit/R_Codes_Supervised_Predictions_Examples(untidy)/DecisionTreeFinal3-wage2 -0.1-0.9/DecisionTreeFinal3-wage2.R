...
lesdonnees <- read.csv("C:\\Users\\Fatihi\\Desktop\\data\\1LAB\\DataFinal\\wageafford2.csv")

#converting column(s) to factor 
lesdonnees$afford1 <- as.factor(lesdonnees$afford1)
...
set.seed(123)
index <- sample(2, nrow(lesdonnees), prob = c(0.6, 0.4), replace = TRUE) #2, : generating samples of ones and twos. nrow(x) means supplying the number of rows in x to generate. prob = c(0.6,0.4) splitting the dataset. Replace = TRUE means that the index will stay the same when used for later.
...
#Training the data
lesdonnees_model <- rpart(formula = afford1 ~ ., data = lesdonnees_train, method = "class") # afford1 ~., imply we're using all the previous column to as measure to train
...
confusionMatrix(data = class_predicted, reference = lesdonnees_test$afford1) #

#Model accuraty on test data
accuracy(actual= lesdonnees_test$afford1, predicted = class_predicted)

#Tree splitting criteria based comparison#

#Model training based on gini-based splitting criteria
lesdonnees_model1 <- rpart(formula = afford1 ~., data = lesdonnees_train, method = "class", parms = list(split = "gini"))

#Model training based on information gain-based splitting criteria
lesdonnees_model2 <- rpart(formula = afford1 ~., data = lesdonnees_train, method = "class", parms = list(split = "information"))
...
accuracy(actual = lesdonnees_test$afford1, predicted = pred1)

accuracy(actual = lesdonnees_test$afford1, predicted = pred2)
...
accuracy(actual = lesdonnees_test$afford1, predicted = pred3) #not much change in the accuracy even after pruning
...
for(i in 1:num_models){
  ...
  lesdonnees_models[[i]] <- rpart(formula = afford1 ~., data = lesdonnees_train, method = "class", minsplit = minsplit, maxdepth = maxdepth)
...
for (i in 1:num_models){
  ...
  accuracy_values[i] <- accuracy(actual = lesdonnees_test$afford1, predicted = pred)
}
...
accuracy(actual = lesdonnees_test$afford1, predicted = pred)
...
for(i in 1:num_models){
  ...
  #Train a model and store in the list
  lesdonnees_models[[i]] <- rpart(formula = afford3 ~., data = lesdonnees_train, method = "class", minsplit = minsplit, maxdepth = maxdepth)
...
#Training the data
lesdonnees_model <- rpart(formula = afford4 ~ ., data = lesdonnees_train, method = "class") # afford4 ~., imply we're using all the previous column to as measure to train
...
#Generate a confusion matrix for the test data
confusionMatrix(data = class_predicted, reference = lesdonnees_test$afford4) #

#Model accuraty on test data
accuracy(actual= lesdonnees_test$afford4, predicted = class_predicted)

#Tree splitting criteria based comparison#

#Model training based on gini-based splitting criteria
lesdonnees_model1 <- rpart(formula = afford4 ~., data = lesdonnees_train, method = "class", parms = list(split = "gini"))

#Model training based on information gain-based splitting criteria
lesdonnees_model2 <- rpart(formula = afford4 ~., data = lesdonnees_train, method = "class", parms = list(split = "information"))
...
#Compare classification accuracy on test data
accuracy(actual = lesdonnees_test$afford4, predicted = pred1)

accuracy(actual = lesdonnees_test$afford4, predicted = pred2)
...
accuracy(actual = lesdonnees_test$afford4, predicted = pred3) #not much change in the accuracy even after pruning
...
for(i in 1:num_models){
  ...
  #Train a model and store in the list
  lesdonnees_models[[i]] <- rpart(formula = afford4 ~., data = lesdonnees_train, method = "class", minsplit = minsplit, maxdepth = maxdepth)
...
for (i in 1:num_models){
  ...
  #Compute test accuracy and add to the empty vector accuracy values
  accuracy_values[i] <- accuracy(actual = lesdonnees_test$afford4, predicted = pred)
}
...
#Best_model accuracy on test data
pred <- predict(object = best_model, newdata =  lesdonnees_test, type = "class")
accuracy(actual = lesdonnees_test$afford4, predicted = pred)
