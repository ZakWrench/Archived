...
Diabetes <- read.csv("C:\\Users\\Fatihi\\Desktop\\data\\HS\\Interested\\interest.csv")
...
Diabetes_test$INTEREST <- as.factor(Diabetes_test$INTEREST)
...
accuracy(actual = Diabetes_test$INTEREST, predicted = pred3) #not much change in the accuracy even after pruning
...
for(i in 1:num_models){
  ...
  diabetes_models[[i]] <- rpart(formula = INTEREST ~., data = Diabetes_train, method = "class", minsplit = minsplit, maxdepth = maxdepth)
...
for (i in 1:num_models){
  ...
  accuracy_values[i] <- accuracy(actual = Diabetes_test$INTEREST, predicted = pred)
}
...
accuracy(actual = Diabetes_test$INTEREST, predicted = pred)
