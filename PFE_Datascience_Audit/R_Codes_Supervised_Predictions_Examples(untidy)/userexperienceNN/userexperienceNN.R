...
data <- read.csv("C:\\Users\\Fatihi\\Desktop\\data\\1LAB\\DataFinal\\NN\\interest2.csv", header = TRUE)
# Random sampling
...
# fit neural network
set.seed(2)
NN = neuralnet(INTEREST ~., trainNN, hidden = 4, rep = 1 , linear.output = T )
...
for(j in 10:65){
  for (i in 1:k) {
    ...
    NN = neuralnet(INTEREST ~ ., trainNN, hidden = 4, linear.output= T)
